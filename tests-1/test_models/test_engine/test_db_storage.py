...
OBOBOB        obj = State(name="Some state")
        obj.save()
OBOBOB        models.storage.save()
OBOBOB        return_obj = list(models.storage.all(State).values())[0].id
OBOBOB        z = str(models.storage.all()['State.' + return_obj])
        self.assertNotEqual(z, None)

    def test_get_dbstorage2(self):
OBOBOB        """Test get storage engine with valid data"""
